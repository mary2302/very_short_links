...
Для помощи в написании кода проекта и тестов и в разработке архитектуры проекта использовалась LLM GPT-4.1.
...
```
### Деплой осуществлен по ссылке:  https://very-short-links.onrender.com/docs
...
- 100 пользователей, 10 новых пользователей в секунду, 60 секунд.
- Всего обработано более 6000 запросов.
- Среднее время ответа: **12-40 мс** для GET-запросов (редирект, статистика, поиск).
- Ошибок < 2% (в основном из-за некорректных данных или авторизации).

**Преимущества кэширования Redis:**
- Кэширование позволяет мгновенно получать оригинальный URL по короткому коду без обращения к базе данных.
- При высокой нагрузке (100+ пользователей) система стабильно выдерживает поток запросов, не теряя производительность.
- Кэширование статистики переходов снижает нагрузку на PostgreSQL и ускоряет обновление счетчиков.
- Кэш Redis обеспечивает быстрый доступ к часто используемым ссылкам и уменьшает задержки при массовых редиректах.

**Вывод:**
- Сервис "Very short links" устойчив к нагрузке и масштабируется благодаря использованию Redis.
- Кэширование критично для быстрого редиректа и массового доступа.
- Производительность подтверждена нагрузочными тестами.
